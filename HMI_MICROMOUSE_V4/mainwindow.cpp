...
{
    return (index == 1) ? SUPERVISOR_RUN_MODE_GO_A_TO_B : SUPERVISOR_RUN_MODE_FIND_CELLS;
}
...
void setDetectionLabel(QLabel *label, bool active)
{
    ...
    label->setStyleSheet(active ? "background-color: #22c55e; border: 1px solid #15803d; border-radius: 4px;"
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), serialPort(new QSerialPort(this)), m_parser(new UnerbusParser(this)),
      udpSocket(nullptr), sensorUpdateTimer(new QTimer(this)), pwmUpdateTimer(new QTimer(this)),
      primitiveTestUpdateTimer_(new QTimer(this))
{
    ...
    navigationButtonGroup->addButton(ui->btnLabyrinth, 5); // ID 5 para la página de visual del laberinto
    ...
    connect(primitiveTestUpdateTimer_, &QTimer::timeout, this, &MainWindow::requestPrimitiveTestStatus);
...
void MainWindow::on_btnConnectSerie_clicked()
{
    ...
    if (serialPort->open(QIODevice::ReadWrite))
    {
        ...
        QTimer::singleShot(200, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_PWM_PERIOD); });
        QTimer::singleShot(300, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_ROBOT_STATUS); });
    }
    else
    {
...
void MainWindow::onPacketReceived(quint8 command, const QByteArray &payload)
{
    ...
    QString commandName = commandNameCStr ? QString(commandNameCStr)
                                          : QString("Desconocido (0x%1)").arg(command, 2, 16, QChar('0').toUpper());

    // Formateamos el mensaje y lo mostramos en el QPlainText "commsLog"
    QString logMessage = QString("Recibido CMD: %1, Payload: %2").arg(commandName).arg(QString(payload.toHex(' ')));
    ...
    case Unerbus::CommandId::CMD_UPDATE_MAZE_CELL:
    {
        if (payload.size() >= 4)
        {
            ...
            if (logical_x >= 0 && logical_x < MAZE_WIDTH && logical_y >= 0 && logical_y < MAZE_HEIGHT)
            {
...
void MainWindow::populateCMDComboBox()
{
    ...
    ui->CMDComboBox->addItem(
        "GET_IR_SENSOR_SNAPSHOT (0xA0)", static_cast<quint8>(Unerbus::CommandId::CMD_GET_IR_SENSOR_SNAPSHOT));
    ...
    ui->CMDComboBox->addItem(
        "GET_LOCAL_IP_ADDRESS (0xE0)", static_cast<quint8>(Unerbus::CommandId::CMD_GET_LOCAL_IP_ADDRESS));
    ui->CMDComboBox->addItem(
        "SET_UART_BYPASS_CONTROL (0xDD)", static_cast<quint8>(Unerbus::CommandId::CMD_SET_UART_BYPASS_CONTROL));
    ui->CMDComboBox->addItem("GET_PID_GAINS (0x41)", static_cast<quint8>(Unerbus::CommandId::CMD_GET_PID_GAINS));
    ui->CMDComboBox->addItem(
        "GET_MAX_PWM_CORRECTION (0x43)", static_cast<quint8>(Unerbus::CommandId::CMD_GET_MAX_PWM_CORRECTION));
    ui->CMDComboBox->addItem(
        "GET_MOTOR_BASE_SPEEDS (0x45)", static_cast<quint8>(Unerbus::CommandId::CMD_GET_MOTOR_BASE_SPEEDS));
    ui->CMDComboBox->addItem(
        "GET_TURN_PID_GAINS (0x49)", static_cast<quint8>(Unerbus::CommandId::CMD_GET_TURN_PID_GAINS));
    ui->CMDComboBox->addItem(
...
void MainWindow::updateIrSensorsUI(const QByteArray &payload)
{
    ...
    quint16 ir_front_left, ir_front_right, ir_diag_left, ir_diag_right, ir_side_left, ir_side_right, ir_gnd_front,
        ir_gnd_rear;
    quint8 detection_flags;

    stream >> ir_side_right >> ir_diag_right >> ir_front_right >> ir_gnd_front >> ir_front_left >> ir_diag_left >>
        ir_side_left >> ir_gnd_rear >> detection_flags;
    ...
    setDetectionLabel(ui->lblWallFrontDetected, (detection_flags & SENSOR_DET_WALL_FRONT) != 0);
    setDetectionLabel(ui->lblWallLeftDetected, (detection_flags & SENSOR_DET_WALL_LEFT) != 0);
    setDetectionLabel(ui->lblWallRightDetected, (detection_flags & SENSOR_DET_WALL_RIGHT) != 0);
    setDetectionLabel(ui->lblWallDiagLeftDetected, (detection_flags & SENSOR_DET_WALL_DIAG_LEFT) != 0);
    setDetectionLabel(ui->lblWallDiagRightDetected, (detection_flags & SENSOR_DET_WALL_DIAG_RIGHT) != 0);
    setDetectionLabel(ui->lblFloorFrontDetected, (detection_flags & SENSOR_DET_FLOOR_FRONT) != 0);
    setDetectionLabel(ui->lblFloorRearDetected, (detection_flags & SENSOR_DET_FLOOR_REAR) != 0);
}
...
void MainWindow::setupControlPage()
{
    ...
    connect(ui->spinBoxRightMotorPos, QOverload<int>::of(&QSpinBox::valueChanged), ui->hSliderRightMotorPos,
        &QSlider::setValue);
    connect(ui->hSliderRightMotorNeg, &QSlider::valueChanged, ui->spinBoxRightMotorNeg, &QSpinBox::setValue);
    connect(ui->spinBoxRightMotorNeg, QOverload<int>::of(&QSpinBox::valueChanged), ui->hSliderRightMotorNeg,
        &QSlider::setValue);
    connect(ui->hSliderLeftMotorPos, &QSlider::valueChanged, ui->spinBoxLeftMotorPos, &QSpinBox::setValue);
    connect(ui->spinBoxLeftMotorPos, QOverload<int>::of(&QSpinBox::valueChanged), ui->hSliderLeftMotorPos,
        &QSlider::setValue);
    connect(ui->hSliderLeftMotorNeg, &QSlider::valueChanged, ui->spinBoxLeftMotorNeg, &QSpinBox::setValue);
    connect(ui->spinBoxLeftMotorNeg, QOverload<int>::of(&QSpinBox::valueChanged), ui->hSliderLeftMotorNeg,
        &QSlider::setValue);
...
void MainWindow::drawMaze()
{
    ...
    for (int i = 0; i < MAZE_WIDTH; i++)
    {
        ...
        QGraphicsTextItem *leftText = new QGraphicsTextItem(QString::number(logical_y_label), leftAnchor);
        leftText->setFont(numberFont);
        leftText->setDefaultTextColor(textPen.color());
        leftText->setPos(-leftText->boundingRect().width() / 2.0, -leftText->boundingRect().height() / 2.0);
    }
    ...
    for (int x = 0; x < MAZE_WIDTH; x++)
    {
        for (int y = 0; y < MAZE_HEIGHT; y++)
        {
            ...
            if (cellData & CELL_VISITED)
            {
                ...
                {
                    QGraphicsLineItem *w = mazeScene->addLine(px, py, px + cellSize, py, wallPen);
                    w->setZValue(8);
                }
                ...
                {
                    QGraphicsLineItem *w = mazeScene->addLine(px, py + cellSize, px + cellSize, py + cellSize, wallPen);
                    w->setZValue(8);
                }
                ...
                {
                    QGraphicsLineItem *w = mazeScene->addLine(px + cellSize, py, px + cellSize, py + cellSize, wallPen);
                    w->setZValue(8);
                }
                ...
                {
                    QGraphicsLineItem *w = mazeScene->addLine(px, py, px, py + cellSize, wallPen);
                    w->setZValue(8);
                }
            }
        }
...
void MainWindow::on_btnGetPidNavConfig_clicked()
{
    ...
    // saturar el micro
    QTimer::singleShot(100, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_MAX_PWM_CORRECTION); });
    QTimer::singleShot(200, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_WALL_THRESHOLDS); });
    QTimer::singleShot(300, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_WALL_TARGET_ADC); });
}
...
void MainWindow::on_btnGetPidTurnConfig_clicked()
{
    ...
    QTimer::singleShot(100, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_TURN_MAX_SPEED); });
    QTimer::singleShot(200, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_PIVOT_TURN_DPS); });
    QTimer::singleShot(300, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_SMOOTH_TURN_CONFIG); });
    QTimer::singleShot(
        400, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_TURN_VELOCITY_PID_GAINS); });
    QTimer::singleShot(500, this, [this]() { sendUnerbusCommand(Unerbus::CommandId::CMD_GET_TURN_TARGET_DPS); });
}
...
void MainWindow::on_btnSetPidTurnConfig_clicked()
{
    ...
    QTimer::singleShot(200, this,
        [this]()
        {
            ...
            pivotTurnDpsStream << static_cast<quint16>(ui->editPivotTurnDps->text().toUShort());
            sendUnerbusCommand(Unerbus::CommandId::CMD_SET_PIVOT_TURN_DPS, pivotTurnDpsPayload);
        });
...
void MainWindow::sendApproachFrontWallTarget()
{
    ...
    stream.setByteOrder(QDataStream::LittleEndian);
    stream << target;
    sendUnerbusCommand(Unerbus::CommandId::CMD_SET_APPROACH_FRONT_WALL_TARGET, payload);
}
...
void MainWindow::updateSupervisorInitialPoseUI(const QByteArray &payload)
{
    ...
    if ((x >= MAZE_WIDTH) || (y >= MAZE_HEIGHT) || (heading > static_cast<quint8>(HEADING_WEST)))
    {
        return;
    }
...
void MainWindow::updateSupervisorDebugStatusUI(const QByteArray &payload)
{
    ...
    lblSupervisorState->setText(QString("%1 (%2)").arg(supervisorStateToText(state)).arg(state));
    lblSupervisorAction->setText(QString("%1 (%2)").arg(supervisorActionToText(action)).arg(action));
    lblSupervisorResult->setText(QString("%1 (%2)").arg(supervisorResultToText(result)).arg(result));
...
void MainWindow::updatePrimitiveTestConfig(const QByteArray &payload)
{
    ...
    if ((responseType != PRIM_TEST_RESP_CONFIG) || (testType != PRIM_TEST_SMOOTH_TURN))
    {
        return;
    }
